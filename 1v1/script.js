...
function getLegels(name, index) {
    ...
    if (name === "pawn-b") {
        ...
        // Move forward two (rank 7)
        if (index >= 8 && index <= 15 &&
            !isOccupiedWhite(twoForward) &&
            !isOccupiedBlack(twoForward))
            legalIndexes.push(twoForward);
    ...
    if (name === "knight-b") {
        ...
        for (const moveOffset of knightMoves) {
            ...
            if ((Math.abs(moveRow - row) === 2 && Math.abs(moveCol - col) === 1) ||
                (Math.abs(moveRow - row) === 1 && Math.abs(moveCol - col) === 2)) {
                if (!isOccupiedBlack(move))
                    legalIndexes.push(move);
            }
        }
    }
    ...
    if (name === "king-b") {
        ...
        // Black castling (assuming you add black versions of flags)
        if (!blackKingHasMoved && !blackKingsideRookHasMoved) {
            if (!isOccupiedWhite(5) && !isOccupiedBlack(5) &&
                !isOccupiedWhite(6) && !isOccupiedBlack(6)) {
                legalIndexes.push(6); // g8
            }
        }
        if (!blackKingHasMoved && !blackQueensideRookHasMoved) {
            if (!isOccupiedWhite(1) && !isOccupiedBlack(1) &&
                !isOccupiedWhite(2) && !isOccupiedBlack(2) &&
                !isOccupiedWhite(3) && !isOccupiedBlack(3)) {
                legalIndexes.push(2); // c8
            }
        }
    }
